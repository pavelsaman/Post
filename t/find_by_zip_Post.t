...
my $res = eval{ $post->$method({ zip => "10003"}) };
my $error = $@;
like($error, qr/hash ref passed/);

# fails if an array ref is passed
my $res = eval{ $post->$method(["10003"]) };
my $error = $@;
like($error, qr/array ref passed/);

###############################################################################
# find_by_zip() returns a list of values
...
my $branches = $post->$method(q{10003});
is(ref $branches, ref []);
# returns only 1 value
is(@$branches, 2);
# correct value
