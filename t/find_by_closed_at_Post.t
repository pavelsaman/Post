...
###############################################################################
# returns 2 closed branches on Sat at 10a.m.

my $branches = $post->$method({ Day => "Saturday", Time => "10:00" });

is(ref $branches, ref []);
is(@$branches, 2);

###############################################################################
# returns 1 closed branch on Friday at 7a.m.

my $branches = $post->$method({ Day => "Friday", Time => "07:00" });

is(ref $branches, ref []);
is(@$branches, 1);

###############################################################################
# returns 1 closed branch on Wednesday during a day break

my $branches = $post->$method({ Day => "Wednesday", Time => "12:30" });
